...
// Import ang Formats para siguradong QR code lang ang hahanapin
...
export default function Scanner() {
    ...
    useEffect(() => {
        // Siguraduhing isang instance lang ng scanner ang nagagawa at naka-set lang sa QR_CODE
        ...
        const startScanner = async () => {
            ...
            // Scanner Settings: Mas magaan para sa laptop processor
            const scanConfig = { 
                fps: 15, // Ginawang 15 para stable at di uminit/maglag ang browser
                qrbox: { width: 250, height: 250 },
                aspectRatio: 1.0, 
                disableFlip: true // Set to true kasi nakakapagpabagal yung pag-mirror ng video
            };
            ...
            const onScanSuccess = async (decodedText: string) => {
                ...
                try {
                    ...
                    // Ginawang 1 second (1000ms) ang cooldown bago magbasa ulit
                    setTimeout(() => {
            ...
            // Error Function habang naghahanap ng QR
            const onScanFailure = (errorMessage: string) => {
                // Ignore
            };

            try {
                // DIRETSONG FRONT CAMERA: Wala nang try/catch loop na nagpapabagal
                await scanner.start(
                    { facingMode: "user" }, // "user" = Front / Web Camera
                    scanConfig, 
                    onScanSuccess, 
                    onScanFailure
                );
                console.log("Started laptop camera immediately.");
            } catch (err) {
                console.error("Failed to start camera", err);
                setFeedback({ message: "Camera access denied. Please allow camera permissions.", type: "error" });
            }
        };
        ...
        <div className="w-full flex flex-col items-center justify-center h-[calc(100vh-6rem)]">
            ...
            <style>
                {`
                    ...
                    #reader video { border-radius: 1rem; object-fit: cover; transform: scaleX(-1); } /* CSS flip if needed visually lang, not affecting decode speed */
                `}
            </style>
